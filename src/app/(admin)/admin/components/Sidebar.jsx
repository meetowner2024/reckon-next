...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`w-64 bg-[#48ADB9] text-white h-screen fixed transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 z-40`}>
        <div className="p-4">
                  ...
                  <>
                    <button
                      ...
                      className={`w-full text-left p-2 rounded flex justify-between items-center ${
                        pathname.startsWith(item.href) ? 'bg-gray-700 text-white' : 'hover:bg-gray-700'
                      }`}
                    >
                      {item.label}
                          ...
                          <li key={sub.href}>
                            <Link
                              ...
                              className={`block p-2 rounded ${
                                pathname === sub.href ? 'bg-gray-600' : 'hover:bg-gray-600'
                              }`}
                            >
                              {sub.label}
                            </Link>
