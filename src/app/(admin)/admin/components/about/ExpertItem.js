...
export default function ExpertItem({ expert, index, onDelete, onUpdate }) {
  ...
  if (editing) {
    return (
      <div className="p-4 border rounded space-y-3 bg-yellow-50">
        ...
        <input value={role} onChange={(e) => setRole(e.target.value)} className="w-full p-2 border rounded" />
      ...
      <div className="flex gap-3">
        {expert.image && (
          <div className="w-20 h-20 border rounded overflow-hidden">
            <Image src={expert.image} alt="Expert" width={80} height={80} className="object-cover" />
          </div>
        )}
        <div>
