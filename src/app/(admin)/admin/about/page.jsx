...
export default function AddCompanyForm({ onSuccess }) {
  ...
  const addCard = () => setCards((c) => [...c, { id: Date.now().toString(), image: null, preview: null, title: "", description: "" }]);
  const removeCard = (id) => setCards((c) => c.filter((x) => x.id !== id));

  const addExpert = () => setExperts((e) => [...e, { id: Date.now().toString(), image: null, preview: null, name: "", role: "" }]);
  ...
  return (
    <form onSubmit={submit} className="space-y-10 max-w-4xl mx-auto bg-white shadow-md rounded-xl p-8 border border-gray-200">
      ...
      <div className="space-y-4">
        <input type="text" placeholder="Main Title" value={title} onChange={(e) => setTitle(e.target.value)} className="w-full border rounded-lg p-3 focus:ring-2 focus:ring-blue-500 outline-none" />
        <textarea placeholder="Main Description..." value={description} onChange={(e) => setDescription(e.target.value)} rows={4} className="w-full border rounded-lg p-3 focus:ring-2 focus:ring-blue-500 outline-none" />
        <div>
      ...
      <div className="space-y-4">
        ...
        {cards.map((c, i) => (
          <div key={c.id} className="p-4 border rounded-lg bg-gray-50 space-y-3 relative">
            {cards.length > 1 && (
    ...
    <div className="space-y-4">
      <div>
        ...
        <input
          ...
          className="w-full px-4 py-3 text-xl font-bold text-center bg-white border border-blue-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition"
        />
      </div>
      <div>
        ...
        <textarea
          ...
          rows={3}
          className="w-full px-4 py-3 text-sm text-gray-700 bg-white border border-blue-300 rounded-lg resize-none focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition"
        />
      </div>
    </div>
  </div>
  ...
  <div className="grid gap-5">
    {experts.map((ex, i) => (
      ...
      <div className="flex flex-col items-center pt-4 border-t space-y-2">
        <button disabled={saving} className="flex items-center gap-2 px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium disabled:opacity-50">
          <Save size={18} /> {saving ? "Saving..." : "Save All"}
        </button>
        {msg && <p className={`text-sm font-medium ${msg.includes("Saved") ? "text-green-600" : "text-red-600"}`}>{msg}</p>}
      </div>
    </form>
  );
}